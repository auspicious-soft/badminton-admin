  ...
  return (
    <div className=" px-4 py-6">
      ...
      <div className="flex flex-col lg:flex-row bg-[#f2f2f4] rounded-[20px] p-4 gap-6">
        ...
        {/* Right Section */}
        <div className="w-full lg:w-1/2 bg-white rounded-[20px] p-6">
          <h2 className="text-[#10375c] text-xl font-medium mb-6">Teams Joined</h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {[
              "Chance & Brandon", "Charlie & Madelyn", "Jocelyn & Kadin",
              "Corey & Kierra", "Skylar & Carter", "Cooper & Giana"
            ].map((team, index) => (
              <div 
                key={index}
                className="flex items-center bg-[#0E3A5A] text-white rounded-full px-4 py-2"
              >
          ...
          <div className="bg-[#f2f2f4] rounded-[10px] p-5">
            <div className="space-y-6">
              {[1, 2, 3, 4].map((index) => (
                <div key={index} className="flex flex-col md:flex-row gap-2.5">
                  <div className="w-full md:w-48 space-y-1 relative">
                    ...
                    <button 
                      className="w-full h-[45px] px-4 bg-white rounded-[28px] flex justify-between items-center text-black/60 text-xs font-medium"
                        ...
                        {games.map((game) => (
                          <label key={game} className="flex items-center gap-2 cursor-pointer text-[#1b2229] text-sm font-medium">
                            <input
                              ...
                              name={`game-${index}`}
                              value={game}
                              checked={selectedGame[index] === game}
                              onChange={() => selectGame(index, game)}
                            />
                            {game}
                          </label>
                        ))}
                  ...
                  <div className="w-full md:w-[193px] space-y-1">
                    ...
                    <input 
                      type="datetime-local" 
                      className="w-full h-[45px] px-4 bg-white rounded-[28px] text-black/60 text-xs font-medium"
                    />
                  </div>
              ...
              <button className="w-full md:w-auto px-8 py-4 bg-[#10375c] rounded-[28px] text-white text-sm font-medium mx-auto block">
                Add More
              </button>
            </div>
          </div>
