...
export default function MatchesComponent({ name,selectedGame,selectedCity, selectedDate }: { name: string, selectedGame:string, selectedCity:string, selectedDate:string }) {
  ...
  const { data, mutate, isLoading, error } = useSWR(
    `/admin/get-matches?page=${page}&limit=${itemsPerPage}&search=${searchParams}&type=${type}&game=${selectedGame}&date=${selectedDate}`,
    getAllMatches
  );
  // // Fetch matches with SWR, updating the URL with page and search params
  // const { data, mutate, isLoading, error } = useSWR(
  //   `/admin/get-matches?page=${page}&limit=${itemsPerPage}&search=${searchParams}&type=completed&game=all&date=`,
  //   getAllMatches
  // );
  ...
  // Debug API response and pagination metadata
