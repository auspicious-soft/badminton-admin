...
const MaintenanceModal: React.FC<MaintenanceModalProps> = ({ isOpen, onClose, onSubmit }) => {
    ...
    const { data: session } = useSession();
    const userRole = (session as any)?.user?.role;
    const userVenueId = (session as any)?.user?.venueId; // Get the venueId for employee
    const { data: venueData } = useSWR("admin/venue-list", getVenueForMaintenance);
    ...
    useEffect(() => {
        if (userRole === 'employee' && userVenueId && venueData?.data?.data) {
            ...
            if (employeeVenue) {
                setValue('venue', userVenueId); // Set the venue field to the employee's venueId
    ...
    const handleFormSubmit = async (data: any) => {
        ...
        try {
            ...
            // Reset all fields except 'venue' for employees
            reset({
                venue: watchedVenue, // Preserve current venue value
                court: '',
                date: '',
                timeslots: [],
                reason: '',
            ...
            // Reset all fields for non-employees
            reset();
        }
            onClose();
        } catch (error: any) {
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)} className="w-full flex flex-col gap-2">
                    <div>
                        ...
                        <select
                            {...register('venue')}
                            className="w-full h-[45px] px-4 py-2 bg-[#f2f2f4] rounded-full text-black/60 text-sm font-medium outline-none"
                            disabled={userRole === 'employee'} // Disable for employees
                        >
                            <option value="">Select</option>
                            {venueOptions.map((venue) => (
                                <option key={venue.value} value={venue.value}>
