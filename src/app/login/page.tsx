  ...
  useEffect(() => {
    if (session) {
      if ((session as any)?.user?.role === 'admin') {
        ...
        if (isOnboarded && verified === 'Active') router.push('/admin/dashboard')
        else router.push('/onboarding')
      }
      else if ((session as any)?.user?.role === 'employee') {
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    startTransition(async () => {
      ...
      if (resss?.success) {
        ...
        if (resss?.data?.user?.role === 'employee') {
          window.location.href = '/employee/dashboard'
        }
        else if (resss?.data?.user?.role === 'admin') {
          const isOnboarded = resss?.data?.user?.onboardingCompleted
          const verified = resss?.data?.user?.onboardingApplication?.status
          if (isOnboarded && verified === 'Active') window.location.href = '/admin/dashboard'
  ...
  return (
    <div className="container">
      ...
      <div className="grid md:grid-cols-2 gap-8 md:gap-3 lg:gap-0 lg:grid-cols-[minmax(0,_7fr)_minmax(0,_5fr)] items-center pt-6 pb-10">
        ...
        <div>
          <p className="mb-5 md:mb-10 text-center"><span className="inline-block "> <AppLogoIcon /> </span></p>
